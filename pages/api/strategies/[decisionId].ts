...
import { getDecisionForWorkflow, listStrategicDecisionLogEntries as listPostgresStrategicDecisionLogEntries } from "../../../src/store/postgres";

type StrategyStatus = "Proposed" | "In Review" | "Approved" | "Blocked";
...
interface StrategyResponseEntry {
  id: string;
  name: string;
...
function buildFallbackStrategyFromWorkflow(decisionId: string, workflowDecision: NonNullable<Awaited<ReturnType<typeof getDecisionForWorkflow>>>): StrategyResponseEntry {
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<StrategyDetailsResponse>): Promise<void> {
  ...
  try {
    const [entries, workflowDecision] = await Promise.all([
      ...
      getDecisionForWorkflow(decisionId.trim()),
    ]);

    const entry = entries.find((candidate) => normalizeId(candidate.id) === normalizedDecisionId);
    const resolvedEntry = entry ?? (workflowDecision ? buildFallbackStrategyFromWorkflow(decisionId.trim(), workflowDecision) : null);
  ...
  res.status(404).json({
    ...
    details: `No entry found for ${decisionId.trim()}.`,
  });
}
