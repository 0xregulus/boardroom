...
const OFFLINE_GOVERNANCE_CHECKS = {
...
function offlineReviewResponse(provider: LLMProvider, request: LLMCompletionRequest): string {
  ...
  return JSON.stringify({
    agent,
    thesis: `${agent} offline review: evidence suggests a viable path with measurable downside controls.`,
    score,
    ...
    citations: [
      {
        url: `https://offline.local/${provider.toLowerCase()}/${agent.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`,
    ...
    apga_impact_view: "Offline indicates moderate upside with controllable downside exposure.",
    governance_checks_met: OFFLINE_GOVERNANCE_CHECKS,
  });
}

function offlineChairpersonResponse(provider: LLMProvider, request: LLMCompletionRequest): string {
  ...
  return JSON.stringify({
    executive_summary: "Offline mode generated a synthetic board synthesis for local UX validation.",
...
export async function offlineCompletion(provider: LLMProvider, request: LLMCompletionRequest): Promise<string> {
  const delayMs = resolveOfflineDelayMs(`${provider}:${request.model}:${request.userMessage.slice(0, 180)}`);
  await sleepMs(delayMs);

  if (!request.requireJsonObject) {
    return `[OFFLINE:${provider}] ${request.model} response for local UX testing.`;
  }

  return isChairpersonPrompt(request)
    ? offlineChairpersonResponse(provider, request)
    : offlineReviewResponse(provider, request);
}
